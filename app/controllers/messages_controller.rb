class MessagesController < ApplicationController
  ...
  def index
    #need inner select statement to find MAX id (higher id means more recent message) messages by each user and then use those id's in this where statement
    @most_recent_messages_ids_by_each_user_with_user_id = Message.select("MAX(id) AS id")
    ...
    @most_recent_messages_ids_by_each_user_without_user_id = "SELECT id from (#{@most_recent_messages_ids_by_each_user_with_user_id}) AS message_id_with_user_id"
    @messages = Message.joins("JOIN users AS senders ON messages.sender_id = senders.id").joins("JOIN users AS receivers ON messages.receiver_id = receivers.id")
      ...
      .where("messages.id IN (#{@most_recent_messages_ids_by_each_user_without_user_id})")
